    ...
    void Start() {
      for (int i = 0; i < Layers.Length; i++) {
        if (Layers[i].LayerTransform != null && Layers[i].LayerTransform != transform) {
          ...
          Debug.LogWarning("My Name is" + transform.name+" Ensure that the layers that you have allotted are children of CompressibleUI object and have UI Elements in them!");
        }
      }
